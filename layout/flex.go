...
func flexLayout(context *LayoutContext, box_ Box, maxPositionY pr.Float, skipStack *bo.SkipStack, containingBlock Box,
	...
	for _, child_ := range children {
		...
		resolvePercentages2(child_, containingBlock, mainFlexDirection)
		...
		if child.MinWidth.Auto() {
			...
			contentSize := pr.Float(minContentWidth(*context, newChild, false))
		...
		if child.Style.GetFlexBasis().String == "content" {
			...
			child.FlexBasis = pr.MaybeFloatToValue(resolveOnePercentage(child.Style.GetFlexBasis(), "flex_basis", pr.Float(availableMainSpace), ""))
		...
		*target = resolveOnePercentage(pr.MaybeFloatToValue(*target), axis, pr.Float(availableMainSpace), "")
		...
		if flexBasis.String != "content" {
			...
			if styleAxis.String == "max-content" {
				...
				if axis == "width" {
					child.FlexBaseSize = pr.Float(maxContentWidth(*context, child_, true))
				} else {
					newChild := child_.Copy()
					if bo.IsParentBox(child_) {
						newChild = bo.CopyWithChildren(child_, child.Children, true, true)
				...
				child.Style[axis] = pr.SToV("auto")
				if axis == "width" {
					child.FlexBaseSize = pr.Float(minContentWidth(*context, child_, true))
				} else {
					newChild := child_.Copy()
					if bo.IsParentBox(child_) {
						newChild = bo.CopyWithChildren(child_, child.Children, true, true)
	...
	for _, line := range flexLines {
		...
		// Step 6 - 9.7.3
		initialFreeSpace := pr.Float(availableMainSpace)
		for _, v := range line.line {
			child := v.box.Box()
			if child.Frozen {
		...
		for !line.allFrozen() {
			...
			remainingFreeSpace := pr.Float(availableMainSpace)
	...
	if box.Style.GetAlignContent() == "stretch" {
		...
		if he := box.Style.GetHeight(); cross == "height" && he.String != "auto" {
			...
			if bo.TypeFlexBox.IsInstance(box_) {
		...
		for _, v := range line.line {
			...
			if alignSelf == "stretch" && getCross(child, cross).String == "auto" {
				...
				if getCross(child, cross).String == "auto" {
					if !(crossMargins[0].Auto() || crossMargins[1].Auto()) {
						crossSize := line.crossSize
						if cross == "height" {
							crossSize -= child.MarginTop.V() + child.MarginBottom.V() +
								child.PaddingTop + child.PaddingBottom + child.BorderTopWidth.V() + child.BorderBottomWidth.V()
						} else {
							crossSize -= child.MarginLeft.V() + child.MarginRight.V() +
								child.PaddingLeft + child.PaddingRight + child.BorderLeftWidth.V() + child.BorderRightWidth.V()
						}
						if cross == "width" {
							child.Width = pr.Float(crossSize)
						} else {
							child.Height = pr.Float(crossSize)
						}
						// TODO: redo layout?
					}
				}
	...
	for index, line := range flexLines {
		...
		for _, v := range line.line {
			...
			if alignSelf == "baseline" && axis == "width" {
				...
				line.lowerBaseline = pr.Max(line.lowerBaseline, child.Baseline)
			}
		}
		for _, v := range line.line {
			child := v.box.Box()
			...
			if crossMargins[0].Auto() {
				autoMargins += 1
			}
			if crossMargins[1].Auto() {
				autoMargins += 1
			}
			if autoMargins != 0 {
				...
				if alignSelf == "flex-end" {
					...
					if cross == "height" {
						child.PositionY += line.lowerBaseline - child.Baseline
					} else {
	...
	for _, line := range flexLines {
		for _, v := range line.line {
			...
			if child.IsFlexItem {
				newChild, childResumeAt := blockLevelLayoutSwitch(
	...
	if bo.TypeInlineFlexBox.IsInstance(box_) {
		if axis == "width" { // and main text direction is horizontal
			...
			if len(box.Children) != 0 {
				val = findInFlowBaseline(box.Children[0], false, nil)
			}
			if val != nil {
				box.Baseline = val.V()
			} else {
