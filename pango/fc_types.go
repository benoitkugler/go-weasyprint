...
type fcPatternHash map[string]*fontconfig.FcPattern // (GHashFunc) FcPatternHash,(GEqualFunc) FcPatternEqual

func (m fcPatternHash) lookup(p fontconfig.FcPattern) *fontconfig.FcPattern { return m[p.Hash()] }

func (m fcPatternHash) insert(p *fontconfig.FcPattern) { m[p.Hash()] = p }
