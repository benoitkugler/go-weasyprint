...
static void
parse.message (*FcConfigParse, FcConfigSeverity severe, const char *fmt, ...)
{
...
static void
FcTypecheckValue (parse *FcConfigParse, FcType value, FcType type)
{
	...
	if (value == FcTypeUnknown)
	    return;
	parse.message ( FcSevereWarning, "saw %s, expected %s",
			 FcTypeName (value), FcTypeName (type));
    }
}

static void
FcTypecheckExpr (parse *FcConfigParse, FcExpr *expr, FcType type)
{
...
static FcTest *
FcTestCreate (parse *FcConfigParse,
	      FcMatchKind   kind,
	      FcQual	    qual,
	      const FcChar8 *field,
...
static FcVStack *
FcVStackCreateAndPush (parse *FcConfigParse)
{
    FcVStack    *new;
...
static FcBool
FcVStackPushString (parse *FcConfigParse, FcVStackTag tag, FcChar8 *string)
{
...
static FcBool
FcVStackPushInteger (parse *FcConfigParse, int integer)
{
    FcVStack    *vstack = FcVStackCreateAndPush (parse);
    if (!vstack)
	return FcFalse;
...
static FcBool
FcVStackPushDouble (parse *FcConfigParse, double _double)
{
    FcVStack    *vstack = FcVStackCreateAndPush (parse);
    if (!vstack)
	return FcFalse;
...
static FcBool
FcVStackPushMatrix (parse *FcConfigParse, FcExprMatrix *matrix)
{
    FcVStack    *vstack;
...
static FcBool
FcVStackPushRange (parse *FcConfigParse, FcRange *range)
{
    FcVStack 	*vstack = FcVStackCreateAndPush (parse);
    if (!vstack)
	return FcFalse;
...
static FcBool
FcVStackPushBool (parse *FcConfigParse, FcBool bool_)
{
    FcVStack    *vstack = FcVStackCreateAndPush (parse);
    if (!vstack)
	return FcFalse;
...
static FcBool
FcVStackPushCharSet (parse *FcConfigParse, FcCharSet *charset)
{
    FcVStack	*vstack;
    if (!charset)
	return FcFalse;
...
static FcBool
FcVStackPushLangSet (parse *FcConfigParse, FcLangSet *langset)
{
    FcVStack	*vstack;
    if (!langset)
	return FcFalse;
...
static FcBool
FcVStackPushName (parse *FcConfigParse, FcMatchKind kind, FcObject object)
{
    FcVStack    *vstack = FcVStackCreateAndPush (parse);
    if (!vstack)
	return FcFalse;
...
static FcBool
FcVStackPushTest (parse *FcConfigParse, FcTest *test)
{
    FcVStack    *vstack = FcVStackCreateAndPush (parse);
    if (!vstack)
	return FcFalse;
...
static FcBool
FcVStackPushExpr (parse *FcConfigParse, FcVStackTag tag, FcExpr *expr)
{
    FcVStack    *vstack = FcVStackCreateAndPush (parse);
    if (!vstack)
	return FcFalse;
...
static FcBool
FcVStackPushEdit (parse *FcConfigParse, FcEdit *edit)
{
    FcVStack    *vstack = FcVStackCreateAndPush (parse);
    if (!vstack)
	return FcFalse;
...
static FcBool
FcVStackPushPattern (parse *FcConfigParse, FcPattern *pattern)
{
    FcVStack    *vstack = FcVStackCreateAndPush (parse);
    if (!vstack)
	return FcFalse;
...
static FcVStack *
FcVStackFetch (parse *FcConfigParse, int off)
{
    FcVStack    *vstack;
...
static FcVStack *
FcVStackPeek (parse *FcConfigParse)
{
...
static void
FcVStackPopAndDestroy (parse *FcConfigParse)
{
    FcVStack	*vstack = parse.vstack;
...
static void
FcVStackClear (parse *FcConfigParse)
{
...
static int
FcVStackElements (parse *FcConfigParse)
{
    int		h = 0;
    FcVStack	*vstack = parse.vstack;
...
static FcBool
psStackPush (parse *FcConfigParse, element FcElement, attr []xml.Attr)
{
    FcPStack   *new;
...
static FcBool
FcPStackPop (parse *FcConfigParse)
{
    FcPStack   *old;

    if (!parse.pstack)
    {
	parse.message ( FcSevereError, "mismatching element");
	return FcFalse;
    }
	...
	while (*attrs)
	{
	    if (attrs[0][0])
	    {
		parse.message ( FcSevereWarning, "invalid attribute '%s'", attrs[0]);
	    }
	    attrs += 2;
	}
    }
...
getAttr (parse *FcConfigParse, const char *attr)
{
    FcChar8 **attrs;
    if (!parse.pstack)
	return 0;
...
static FcChar8 *
_get_real_path_from_prefix(parse *FcConfigParse, const FcChar8 *path, const FcChar8 *prefix)
{
#ifdef _WIN32
    FcChar8 buffer[1000] = { 0 };
#endif
    FcChar8 *parent = nil, *retval = nil;
    ...
    if (parent)
    {
	retval = FcStrBuildFilename (parent, path, nil);
	FcStrFree (parent);
    }
    else
    {
...
FcStartElement(void *userData, const XML_Char *name, attr []xml.Attr)
{
    ...
    if (!psStackPush (parse, element, attr))
    {
	parse.message ( FcSevereError, "out of memory");
	return;
    }
    return;
}
...
static void
FcParseRescan (parse *FcConfigParse)
{
	...
	if (v.tag != FcVStackInteger)
	    parse.message ( FcSevereWarning, "non-integer rescan");
	else
...
static void
FcParseInt (parse *FcConfigParse)
{
    FcChar8 *s, *end;
    int	    l;
	...
	parse.message ( FcSevereError, "\"%s\": not a valid integer", s);
    else
...
static void
FcParseDouble (parse *FcConfigParse)
{
    FcChar8 *s, *end;
    double  d;
    ...
    if (end != s + strlen ((char *) s))
	parse.message ( FcSevereError, "\"%s\": not a valid double", s);
    else
...
static void
FcParseString (parse *FcConfigParse, FcVStackTag tag)
{
    FcChar8 *s;
...
static void
FcParseName (parse *FcConfigParse)
{
    ...
    kind_string = getAttr (parse, "target");
    if (!kind_string)
	kind = FcMatchDefault;
    else
    {
	...
	{
	    parse.message ( FcSevereWarning, "invalid name target \"%s\"", kind_string);
	    return;
	}
    }
...
static void
FcParseMatrix (parse *FcConfigParse)
{
    FcExprMatrix m;
    ...
    {
	parse.message ( FcSevereWarning, "Missing values in matrix element");
	return;
    }
    if (FcPopExpr (parse))
      parse.message ( FcSevereError, "wrong number of matrix elements");
    else
      FcVStackPushMatrix (parse, &m);
}

static void
FcParseRange (parse *FcConfigParse)
{
    FcVStack	*vstack;
    FcRange	*r;
    FcChar32	n[2] = {0, 0};
	...
	if (count < 0)
	{
	    parse.message ( FcSevereError, "too many elements in range");
	    return;
	}
	...
	case FcVStackDouble:
	    ...
	    break;
	default:
	    parse.message ( FcSevereError, "invalid element in range");
	    if (dflag)
		d[count] = 0.0L;
	    else
		n[count] = 0;
    ...
    if (count >= 0)
    {
	parse.message ( FcSevereError, "invalid range");
	return;
    }
    if (dflag)
    {
...
static FcBool
FcConfigLexBool (parse *FcConfigParse, const FcChar8 *bool_)
{
    FcBool  result = FcFalse;

    if (!FcNameBool (bool_, &result))
	parse.message ( FcSevereWarning, "\"%s\" is not known boolean",
			 bool_);
    return result;
}

static void
FcParseBool (parse *FcConfigParse)
{
    FcChar8 *s;
...
static void
FcParseCharSet (parse *FcConfigParse)
{
    FcVStack	*vstack;
	...
	case FcVStackInteger:
	    ...
	    {
		parse.message ( FcSevereWarning, "invalid character: 0x%04x", vstack.u.integer);
	    }
	    else
		n++;
	    break;
	case FcVStackRange:
	    ...
	    if (begin <= end)
	    {
		  ...
		  {
		      parse.message ( FcSevereWarning, "invalid character: 0x%04x", i);
		  }
		  else
		      n++;
	      }
	    ...
	    break;
	default:
		parse.message ( FcSevereError, "invalid element in charset");
		break;
	}
	FcVStackPopAndDestroy (parse);
    }
...
static void
FcParseLangSet (parse *FcConfigParse)
{
    FcVStack	*vstack;
	...
	case FcVStackString:
	    ...
	    {
		parse.message ( FcSevereWarning, "invalid langset: %s", vstack.u.string);
	    }
	    else
		n++;
	    break;
	default:
		parse.message ( FcSevereError, "invalid element in langset");
		break;
	}
	FcVStackPopAndDestroy (parse);
    }
		    ...
		    FcValueBinding  *binding_ret)
{
	...
	{
	    parse.message ( FcSevereWarning, "invalid binding \"%s\"", binding_string);
	    return FcFalse;
	}
    }
    *binding_ret = binding;
...
static void
FcParseFamilies (parse *FcConfigParse, FcVStackTag tag)
{
    FcVStack	*vstack;
	...
	{
	    parse.message ( FcSevereWarning, "non-family");
	    FcVStackPopAndDestroy (parse);
	    continue;
	}
	left = vstack.u.expr;
...
static void
FcParseFamily (parse *FcConfigParse)
{
    FcChar8 *s;
    FcExpr  *expr;
...
static void
FcParseAlias (parse *FcConfigParse)
{
    ...
    if (!FcConfigLexBinding (parse, getAttr (parse, "binding"), &binding))
	return;
    while ((vstack = FcVStackPeek (parse)))
    {
	switch ((int) vstack.tag) {
	case FcVStackFamily:
	    if (family)
	    {
		parse.message ( FcSevereWarning, "Having multiple <family> in <alias> isn't supported and may not work as expected");
	...
	case FcVStackTest:
	    ...
	    break;
	default:
	    parse.message ( FcSevereWarning, "bad alias");
	    break;
	}
	FcVStackPopAndDestroy (parse);
    }
    if (!family)
    {
	parse.message ( FcSevereError, "missing family in alias");
	if (prefer)
	    FcExprDestroy (prefer);
	if (accept)
	    FcExprDestroy (accept);
...
static void
FcParseDescription (parse *FcConfigParse)
{
    const FcChar8 *domain;
    FcChar8 *desc;

    domain = getAttr (parse, "domain");
...
static void
FcParseRemapDir (parse *FcConfigParse)
{
    ...
    if (data[0] == 0)
    {
	parse.message ( FcSevereWarning, "empty font directory name for remap ignored");
	return;
    }
    path = getAttr (parse, "as-path");
    if (!path)
    {
	parse.message ( FcSevereWarning, "Missing as-path in remap-dir");
	return;
    }
    attr = getAttr (parse, "prefix");
    salt = getAttr (parse, "salt");
	...
	if (!FcConfigAddFontDir (parse.config, prefix, path, salt))
	    parse.message ( FcSevereError, "out of memory; cannot create remap data for %s as %s", prefix, path);
    }
    FcStrBufDestroy (&parse.pstack.str);
...
static void
FcParseResetDirs (parse *FcConfigParse)
{
    if (!parse.scanOnly)
    {
	if (!FcConfigResetFontDirs (parse.config))
	    parse.message ( FcSevereError, "Unable to reset fonts dirs");
    }
}
 ...
 */
static FcExpr *
FcPopBinary (parse *FcConfigParse, FcOp op)
{
    FcExpr  *left, *expr = 0, *new;
...
static void
FcParseBinary (parse *FcConfigParse, FcOp op)
{
...
static FcExpr *
FcPopUnary (parse *FcConfigParse, FcOp op)
{
...
static void
FcParseUnary (parse *FcConfigParse, FcOp op)
{
...
static void
FcParseDir (parse *FcConfigParse)
{
	...
	if (!FcConfigAddFontDir (parse.config, prefix, nil, salt))
	    parse.message ( FcSevereError, "out of memory; cannot add directory %s", prefix);
    }
    FcStrBufDestroy (&parse.pstack.str);
...
static void
FcParseCacheDir (parse *FcConfigParse)
{
    const FcChar8 *attr;
    FcChar8 *prefix = nil, *p, *data = nil;
	...
	{
	    parse.message ( FcSevereError, "SHGetFolderPathA failed");
	    goto bail;
	}
	...
	if (!FcConfigAddCacheDir (parse.config, data))
	    parse.message ( FcSevereError, "out of memory; cannot add cache directory %s", data);
    }
    FcStrBufDestroy (&parse.pstack.str);
...
static void
FcParseInclude (parse *FcConfigParse)
{
    FcChar8	    *s;
    const FcChar8   *attr;
...
#endif
    FcChar8	    *prefix = nil, *p;
    FcChar8	    *userdir = nil, *userconf = nil;
    FcRuleSet	    *ruleset;
    FcMatchKind	    k;
    ...
    attr = getAttr (parse, "ignore_missing");
    if (attr && FcConfigLexBool (parse, (FcChar8 *) attr) == FcTrue)
	ignore_missing = FcTrue;
    attr = getAttr (parse, "deprecated");
#ifndef _WIN32
    if (attr && FcConfigLexBool (parse, (FcChar8 *) attr) == FcTrue)
        deprecated = FcTrue;
#endif
	...
	else
	{
	     ...
	     */
	    if (FcStrStr (s, (const FcChar8 *)"conf.d") != nil)
		goto userdir;
	    else
		goto userconf;
	}
...
static void
FcParseTest (parse *FcConfigParse)
{
    const FcChar8   *kind_string;
    FcMatchKind	    kind;
	...
	{
	    parse.message ( FcSevereWarning, "invalid test target \"%s\"", kind_string);
	    return;
	}
    }
    qual_string = getAttr (parse, "qual");
    if (!qual_string)
	qual = FcQualAny;
    else
    {
	...
	{
	    parse.message ( FcSevereWarning, "invalid test qual \"%s\"", qual_string);
	    return;
	}
    }
    name = getAttr (parse, "name");
    if (!name)
    {
	parse.message ( FcSevereWarning, "missing test name");
	return;
    }
    compare_string = getAttr (parse, "compare");
    if (!compare_string)
	compare = FcOpEqual;
    else
    {
	...
	{
	    parse.message ( FcSevereWarning, "invalid test compare \"%s\"", compare_string);
	    return;
	}
    }
    iblanks_string = getAttr (parse, "ignore-blanks");
    if (iblanks_string)
    {
	FcBool f = FcFalse;
    ...
    if (!expr)
    {
	parse.message ( FcSevereWarning, "missing test expression");
	return;
    }
    if (expr.op == FcOpComma)
    {
	parse.message ( FcSevereWarning, "Having multiple values in <test> isn't supported and may not work as expected");
    }
...
static void
FcParseEdit (parse *FcConfigParse)
{
    const FcChar8   *name;
    ...
    if (!name)
    {
	parse.message ( FcSevereWarning, "missing edit name");
	return;
    }
    mode_string = getAttr (parse, "mode");
    if (!mode_string)
	mode = FcOpAssign;
    else
    {
	...
	if (mode == FcOpInvalid)
	{
	    parse.message ( FcSevereWarning, "invalid edit mode \"%s\"", mode_string);
	    return;
	}
    }
    if (!FcConfigLexBinding (parse, getAttr (parse, "binding"), &binding))
	...
	expr != nil)
    {
	parse.message ( FcSevereWarning, "Expression doesn't take any effects for delete and delete_all");
	FcExprDestroy (expr);
	expr = nil;
    }
...
static void
FcParseMatch (parse *FcConfigParse)
{
    ...
    kind_name = getAttr (parse, "target");
    if (!kind_name)
	kind = FcMatchPattern;
    else
    {
	...
	{
	    parse.message ( FcSevereWarning, "invalid match target \"%s\"", kind_name);
	    return;
	}
    }
    while ((vstack = FcVStackPeek (parse)))
...
static void
FcParseAcceptRejectFont (parse *FcConfigParse, element FcElement)
{
    FcVStack	*vstack;
	...
	case FcVStackPattern:
	    ...
	    break;
	default:
	    parse.message ( FcSevereWarning, "bad font selector");
	    break;
	}
	FcVStackPopAndDestroy (parse);
    }
...
static FcValue
FcPopValue (parse *FcConfigParse)
{
    FcVStack	*vstack = FcVStackPeek (parse);
    FcValue	value;
	...
	break;
    default:
	parse.message ( FcSevereWarning, "unknown pattern element %d",
			 vstack.tag);
	break;
    }
    FcVStackPopAndDestroy (parse);
...
static void
FcParsePatelt (parse *FcConfigParse)
{
    FcValue	value;
    ...
    name = (char *) getAttr (parse, "name");
    if (!name)
    {
	parse.message ( FcSevereWarning, "missing pattern element name");
...
static void
FcParsePattern (parse *FcConfigParse)
{
    FcVStack	*vstack;
    FcPattern	*pattern = FcPatternCreate ();
	...
	case FcVStackPattern:
	    ...
	    break;
	default:
	    parse.message ( FcSevereWarning, "unknown pattern element");
	    break;
	}
	FcVStackPopAndDestroy (parse);
    }
		    ...
		    int		    has_internal_subset FC_UNUSED)
{
	...
	parse.message ( FcSevereError, "invalid doctype \"%s\"", doctypeName);
}
					...
					FcBool         load)
{
    ...
    p = xmlCreatePushParserCtxt (&sax, &parse, nil, 0, (const char *) filename);
#else
	...
	if (!buf)
	{
	    message (&parse, FcSevereError, "cannot get parse buffer");
	    goto bail3;
	}
	if (len > BUFSIZ)
	{
