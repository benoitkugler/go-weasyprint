...
func getImage(_token Token, baseUrl string) (Image, error) {
	...
	if !ok {
		...
		parsedUrl, ok := parsed.Content.(Url)
		if !ok {
			log.Fatalln("content should be an url here")
		}
		if !parsedUrl.Internal {
			return UrlImage(parsedUrl.Url), nil
		}
		return nil, nil
	}
...
func checkVarFunction(token Token) (out ContentProperty) {
	name, args := parseFunction(token)
	if name == "" {
		return
	}
	if name == "var" && len(args) > 0 {
		...
		return ContentProperty{Type: "var()", Content: NamedTokens{Name: v, Tokens: args}}
	}
	return
}
