...
//         images = [d for d := range page.descendants() if d.elementTag == "img"]
//         assertEqual(t, all([img.elementTag == "img" for img := range images]))
//         assertEqual(t, all([img.positionX == 10 for img := range images]))
//         pageImages.append(images)
//         del images
//     positionsY = [[img.positionY for img := range images]
//                    for images := range pageImages]
//     assertEqual(t, positionsY == [[10], [10]])
...
//     positionsY = [[img.positionY for img := range images]
//                    for images := range pageImages]
//     assertEqual(t, positionsY == [[10, 40], [10, 40], [10]])

// func TestFloatsPageBreaks4(t *testing.T){
// cp := testutils.CaptureLogs()
// defer cp.AssertNoLogs(t)
//     // last float does ! fit, pushed to next page
//     pages = renderPages(`
//       <style>
//         @page{
//           size: 110px;
...
//     pageDivs = []
//     for page := range pages {
//         divs = [div for div := range page.descendants() if div.elementTag == "div"]
//         assertEqual(t, all([div.elementTag == "div" for div := range divs]))
//         pageDivs.append(divs)
//         del divs
//     }

//     positionsY = [[div.positionY for div := range divs] for divs := range pageDivs]
//     assertEqual(t, positionsY == [[10, 70], [10]])
...
//     positionsY = [[div.positionY for div := range divs] for divs := range pageDivs]
//     assertEqual(t, positionsY == [[10], [10, 30]])

// func TestFloatsPageBreaks6(t *testing.T){
// cp := testutils.CaptureLogs()
// defer cp.AssertNoLogs(t)
//     // center div must be the last element,
//     // but float won"t fit && will get pushed anyway
//     pages = renderPages(`
//       <style>
//         @page{
//           size: 110px;
...
//     positionsY = [[div.positionY for div := range divs] for divs := range pageDivs]
//     assertEqual(t, positionsY == [[10], [10], [10]])

// func TestPreferredWidths1(t *testing.T) {
//   cp := testutils.CaptureLogs()
//   defer cp.AssertNoLogs(t)
//     def getFloatWidth(bodyWidth) {
//         page := renderOnePage(t,`
//           <style>
//             @font-face { src: url(weasyprint.otf); font-family: weasyprint }
//           </style>
...
//     assertEqual(t, getFloatWidth(10) == len("consectetur elit") * 16)
//     // Preferred width:
//     assertEqual(t, getFloatWidth(1000000) == len("Lorem ipsum dolor sit amet,") * 16)

// func TestPreferredWidths2(t *testing.T){
// cp := testutils.CaptureLogs()
// defer cp.AssertNoLogs(t)
//     // Non-regression test:
//     // Incorrect whitespace handling := range preferred width used to cause
...
//     assertEqual(t, isinstance(paragraph.Box().Children[0], boxes.LineBox))
// }
...
//     body := html.Box().Children[0]
//     paragraph, = body.Box().Children
//     assertEqual(t, paragraph.width == 40)
...
// }
//     p1, a, p2 = line1.Box().Children
//     assertEqual(t, p1.width == 6 * 20)
//     assertEqual(t, p1.text == "aa bb ")
//     assertEqual(t, p1.positionX == 0 * 20)
//     assertEqual(t, p2.width == 3 * 20)
//     assertEqual(t, p2.text == " ee")
//     assertEqual(t, p2.positionX == 9 * 20)
//     span, aText = a.Box().Children
//     assertEqual(t, aText.width == 3 * 20  // leading space collapse)
//     assertEqual(t, aText.text == "ddd")
//     assertEqual(t, aText.positionX == 6 * 20)
//     assertEqual(t, span.width == 2 * 20)
//     assertEqual(t, span.Box().Children[0].Box().Children[0].text == "cc")
//     assertEqual(t, span.positionX == 12 * 20)

//     p3, = line2.Box().Children
//     assertEqual(t, p3.width == 2 * 20)
