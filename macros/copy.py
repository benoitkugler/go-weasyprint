...
TEMPLATE_FINAL_TYPE = """
func (v {type_}) Copy() ValidatedProperty {{ return v.Copy3() }}
func (v {type_}) Copy2() CascadedProperty {{ return v.Copy3() }}
"""

TEMPLATE_HEADER_COPY = """func (v {type_}) Copy3() CssProperty {{ {body}"""
...
need_private_copy = {"CustomProperty", "RadialGradient", "LinearGradient"}
...
def generate_body_copy_method(type_: str) -> str:
    ...
    if isValueType((kind, details)):
        ...
        if isValueType(details):
            ...
            if subtype != "ident":
                raise ValueError("%s : slice subtype need copy() method" % type_)
        ...
        for field, subtype in details.items():
            if not isValueType(subtype):
                ...
                if subkind != "ident":
                    raise ValueError("field %s in struct must be a defined type" % str(subtype))
...
code += TEMPLATE_FINAL_TYPE.format(type_=type_)
code = """package properties 
...
while need_private_copy:
    ...
    if not type_ in done:
        ...
        if not det == "interface" or kind==  "builtin":
            body = generate_body_copy_method(type_)
            code += TEMPLATE_HEADER_PRIVATE_COPY.format(type_=type_, body=body)
